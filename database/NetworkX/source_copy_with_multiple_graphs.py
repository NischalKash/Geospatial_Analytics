...
try:
    ...
    if connection.is_connected():
        ...
        if option == 1:
            ...
            transfer_data = sorted(transfer_data, key=itemgetter(2),reverse=True)
            iter_val = 0
            while iter_val<len(transfer_data):
                count = 0
                G = networkx.DiGraph()
                while count<10:
                    if transfer_data[iter_val][0] not in G:
                        G.add_node(transfer_data[iter_val][0])
                    if transfer_data[iter_val][1] not in G:
                        G.add_node(transfer_data[iter_val][1])
                    G.add_edge(transfer_data[iter_val][0],transfer_data[iter_val][1],weight=transfer_data[iter_val][2])
                    count+=1
                    iter_val+=1
